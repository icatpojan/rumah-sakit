...
$("#display").on("click", ".sep", function(event){
  ...
  var url = baseURL + '/vclaim/bynokartu/' + no_peserta + '/2023-08-12?t=' + mlite.token;
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $('input:text[name=tgl_perawatan]').val("2023-08-12");
        $('input:text[name=tgl_registrasi]').val("2023-08-12");
        $('input:text[name=jam_rawat]').val("15:40:45");
        $('#notif').html("<div class=\"alert alert-danger alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
